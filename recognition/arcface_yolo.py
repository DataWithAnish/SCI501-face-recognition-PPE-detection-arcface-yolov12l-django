...
# Quiet Ultralytics console spam so our logs are visible
LOGGER.setLevel(logging.ERROR)

# Make stdout line-buffered so prints appear immediately (even under Django)
if hasattr(sys.stdout, "reconfigure"):
    sys.stdout.reconfigure(line_buffering=True)

# Debug logger: prints to console AND a session file (sessions/<id>/debug.log)
_debug_fh = None
def _debug(msg):
    try:
...
# NOTE: We now fully RESET trackers on every detect frame. Keep detect cadence frequent enough.
DETECT_EVERY_N   = 2     # re-detect faces more often to avoid drift
EMBED_EVERY_M    = 9999  # effectively disable mid-cycle re-embeds; detection refresh handles identity
YOLO_EVERY_N     = 8
TRACKER_TYPE     = "CSRT"  # more robust than MOSSE/KCF on CPU
MIN_FACE_SIZE    = 36
NMS_IOU_MATCH    = 0.5    # Threshold only used in generic helpers
...
TRACK_MAX_MISSED   = 1      # frames to keep a lost track
UNKNOWN_SUPPRESS_IOU = 0.90 # hide Unknown track if it overlaps a kept Known track (not applied in hard reset mode)
...
class FaceTrackManager:
    ...
    def reset_with_boxes(self, frame, detected_boxes):
        """Full reset: destroy all existing tracks and rebuild from detections."""
        self._tracks = {}
        self._next_id = 1
        for box in detected_boxes:
            tid = self._next_id; self._next_id += 1
            self._tracks[tid] = FaceTrack(tid, frame, box)
...
def _reset_session(log_dir=None):
    ...
    # open per-session debug log (line-buffered)
    global _debug_fh
    _debug_fh = open(os.path.join(_session_dir, "debug.log"), "a", buffering=1)
    _debug(f"DEBUG LOG OPENED → {os.path.join(_session_dir, 'debug.log')}")
...
def _enforce_single_owner(tracks, frame_idx):
    ...
    for label, cand in current.items():
        ...
        if owner is None:
            _label_owner[label] = {"track_id": best.id, "last_seen": frame_idx, "conf": float(best.conf)}
        else:
...
# ------------------------
# Recognition on Frames (RESET trackers every detect frame)
# ------------------------
...
def process_frame(frame, db_pack, threshold=0.35, frame_idx=0):
    ...
    if detect_frame:
        ...
        # 3) HARD RESET: rebuild trackers strictly from ArcFace detections
        tracks = _track_mgr.reset_with_boxes(raw, det_boxes)

        # 4) Assign identities immediately using *detected embeddings*
        for tr in tracks:
            # Find the matching detected face by IoU (since we used the same boxes, this is stable)
        ...
        # No detection: just update existing tracks without re-identification
        tracks = _track_mgr.update_only(raw)

        # --- If multiple trackers show the SAME known user id, force an immediate ArcFace refresh ---
        def _has_duplicate_known_labels(_tracks):
            seen = set()
            for _tr in _tracks:
                if _tr.label not in ("Unknown", "Suppressed"):
                    if _tr.label in seen:
                        return True
                    seen.add(_tr.label)
            return False

        if _has_duplicate_known_labels(tracks):
            _debug(f"[Frame {frame_idx}] PANIC REFRESH: duplicate known labels → forcing detect+reset...")
            faces_all = arcface_model.get(raw)
            det_boxes, det_scores, keep_faces = [], [], []
            for f in faces_all:
                score = float(getattr(f, "det_score", 1.0))
                x1, y1, x2, y2 = map(int, f.bbox)
                if score < FACE_DET_MIN_SCORE:
            ...
            # HARD RESET and re-ID immediately
            tracks = _track_mgr.reset_with_boxes(raw, det_boxes)
            for tr in tracks:
                best_i, best_iou = -1, 0.0
                for i, f in enumerate(faces):
    ...
    # --- Enforce single owner per known worker label (display-time suppression) ---
    _enforce_single_owner(tracks, frame_idx)

    # --- Since we *reset* on detect frames, we can also hard-delete any duplicates immediately ---
    _debug(
    f"[Frame {frame_idx}] DETECT RESET: faces_detected={len(faces)} "
    f"trackers_reset={len(tracks)}")
    if detect_frame:
        by_label = {}
        for tr in tracks:
            if tr.label == "Unknown":
        ...
        to_delete = []
        for label, cand in by_label.items():
            best = max(cand, key=_track_score)
            for tr in cand:
                if tr.id != best.id:
                    to_delete.append(tr.id)
        _track_mgr.delete_tracks(to_delete)
        tracks = _track_mgr.all()

    # --- Decide which tracks to render (one per known label + all unknowns) ---
    visible_tracks, visible_ids = _select_visible_tracks(tracks)
    # ---- Debug: per-frame summary ----
    # ---- Debug: per-frame summary ----
    known_names = [tr.label for tr in visible_tracks if tr.label != "Unknown"]
    unknown_count = len(visible_tracks) - len(known_names)
    faces_detected = (len(faces) if isinstance(faces, (list, tuple)) else "NA")  # detector runs only on detect frames
    _debug(
    f"[Frame {frame_idx}] detect={detect_frame} faces_detected={faces_detected} "
    f"trackers_total={len(tracks)} visible_boxes={len(visible_tracks)} "
    f"known={len(known_names)} unknown={unknown_count} names={known_names}")



    # ---------- YOLO detection (duty-cycled, keep stats, no rectangles) ----------
    if frame_idx - _prev_yolo["frame_idx"] >= YOLO_EVERY_N:
        det = yolo_model(raw, verbose=False)[0]
        yolo_stats = {}
        yolo_boxes = []
...
def recognize_from_video_stream(video_path=None, threshold=0.35, log_dir=LOG_BASE_DIR):
    ...
    CHANGES:
    - Trackers are **reset** on every detect frame (DETECT_EVERY_N).
    - Only **ONE box per known worker** is drawn.
    - Unknowns are kept as-is (since they can be multiple different people).
    - **NEW:** If two trackers claim the same known worker on a non-detect frame, we force an immediate detect+reset.
    """
    global latest_stats
    ...
    try:
        while True:
            ...
            _debug(
                f"[Frame {frame_idx}] processed | faces={stats.get('faces')} "
                f"known={stats.get('recognized')} unknown={stats.get('unknown')}")
